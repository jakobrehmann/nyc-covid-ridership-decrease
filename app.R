...
ui <- fluidPage(
  titlePanel("New York City Subway System - Data Analysis"),
  tabsetPanel(
  ...
  tabPanel("Network Analysis - Centrality",  splitLayout(tmapOutput(outputId = "cent1"),tmapOutput(outputId = "cent2")))
  )
)
...
server <- function(input, output, session) { 
  ...
  output$covid <- renderPlot({
    ...
    ggplot(COVID, aes(x = date, y = cases_per_day)) + geom_point() + 
      ...
      #ggtitle() +
      labs(title = "Daily New COVID-19 Infections in New York City, NY",
           subtitle = "Red marker matches date on time slider",
           x="Date", y= "COVID-19 New-Infections")
  })
  ...
  output$cent1 <- renderTmap({
    ...
    tm_shape(edges_sf) + 
      ...
      tm_shape(nodes_sf) +
      tm_dots(col = "cent_degree", id = "stop_name", , title = "Node Centrality - Degree")
  })
  ...
  output$cent2 <- renderTmap({
    ...
    tm_shape(edges_sf) + 
      tm_lines(col = "gray50") +#"centrality") +
      tm_shape(nodes_sf) +
      tm_dots(col = "cent_btwn", id = "stop_name", title = "Node Centrality - Betweenness")
  })
      ...
      if(input$demographic == "Median Income") {
        tm_shape(ny_demographics) +
          ...
          tm_symbols(col = "remaining_ridership", 
                     ...
                     size = 0.05, 
                     alpha = 1, 
                     title.col = "Ridership 2020 vs. 2019 (pct)")
      } else if(input$demographic == "Race: Percent White")  {
        tm_shape(ny_demographics) +
          tm_polygons(col = "race_white", 
                      palette = vir, alpha = 1, border.alpha = 0.5, 
